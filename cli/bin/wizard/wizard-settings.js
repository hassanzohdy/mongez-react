...
for (let app of appsList) {
    const modules = getDirectories(root(`src/modules/${app}`));
    ...
    const addServices = (servicesList, module, servicePath = (serviceName) => module + `/services/${serviceName}`) => {
        for (let serviceFile of servicesList) {
            ...
            services.push({
                ...
                value: servicePath(serviceName, module),
    ...
    for (let module of modules) {
        const moduleServices = getFiles(root(`src/modules/${app}/${module}/services`));

        addServices(moduleServices, module);
    }

    if (existsSync(root(`src/modules/${app}/services`))) {
        addServices(getFiles(root(`src/modules/${app}/services`)), 'Shared Services', serviceName => `services/${serviceName}`);
    }
