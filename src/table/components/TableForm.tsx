...
export default function TableForm(props: any): any {
    ...
    const isLazyForm = formOptions.lazyForm === undefined ? config.get('form.lazyForm', false) : formOptions.lazyForm;

    React.useEffect(() => {
        if (open && isLazyForm && record.id && !lazyRecord.id) {
    ...
    if (isLazyForm) {
        if (!lazyRecord.id && record.id) {
            ...
            content = <formOptions.form index={recordIndex} record={lazyRecord} />;
        }
    } else {
