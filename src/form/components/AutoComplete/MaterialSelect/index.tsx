...
export default function AutoComplete(props: any) {
  ...
  let {
    ...
    lazyLoading,
    groupBy,
    mapItem = mapItemInternally,
    addable = false,
    closeOnSelect,
    onChange,
    loadingText = trans("loading"),
    noOptionsText = trans("noOptions"),
    autoHighlight,
    items,
    name,
    value,
  ...
  const [isLoading, setLoading] = React.useState(() => {
    ...
    if (!(value || defaultValue) && searchable) return false;
  ...
  const adjustValue = (value) => {
    ...
    let itemValue = multiple
      ? getItems(currentItems, value)
      : getItem(currentItems, value);
  ...
  const [currentValue, setValue] = React.useState(
    adjustValue(value || defaultValue)
  );
  ...
  const manageRequestResponse = (response) => {
    ...
    if (value || defaultValue) {
      const comingValue = value || defaultValue;
      setValue(
        multiple ? getItems(items, comingValue) : getItem(items, comingValue)
      );
    }
  };
  ...
  React.useEffect(() => {
    ...
    if (!Is.empty(value)) {
      setValue(
        multiple ? getItems(mappedItems, value) : getItem(mappedItems, value)
      );
    }
  }, [items]);
  ...
  if (groupBy) {
    otherProps.groupBy = (option) =>
      Is.callable(groupBy) ? groupBy(option) : Obj.get(option, groupBy);
  }
  ...
  const optionsList = !Is.empty(exceptValues)
    ? currentItems.filter((item) => !exceptValues.includes(item.value))
    : currentItems;
  ...
  return (
    <>
      ...
      <Autocomplete
        ...
        value={currentValue}
        getOptionDisabled={(item: any) =>
          item.disabled === true ||
          disabled ||
          (limit && multiple && currentValue.length === limit)
        }
        onOpen={() => setOpen(true)}
