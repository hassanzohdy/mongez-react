...
import { trans, getLocalizedText } from './../../../../localization';
...
export default function AutoComplete(props: any) {
    ...
    let { onError = null, defaultValue = null, label, limit, variant = config.get('form.input.variant', 'outlined'), disabled, except, request, loading: baseLoading, InputProps = {}, imageable, none, margin = 'normal', searchable, lazyLoading, groupBy, mapItem = mapItemInternally, addable = false, closeOnSelect, onChange, loadingText = trans('loading'), noOptionsText = trans('noOptions'), autoHighlight, items, name, value, multiple, required, mapResponse = defaultMapResponse, ...otherProps } = props;
    ...
    const [currentValue, setValue] = React.useState(adjustValue(value || defaultValue));
    ...
    const manageRequestResponse = response => {
        ...
        if (value || defaultValue) {
            ...
            setValue(multiple ? getItems(items, comingValue) : getItem(items, comingValue));
        }
    };
