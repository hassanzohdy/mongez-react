...
export default function FileInput(props: FileInputProps) {
    const { label, value, required, accept, onChange, buttonText, buttonIcon, id = Random.id(), name, ...otherProps } = props;
    ...
    return (
        <FileInputWrapper>
            <If condition={required !== true && !Is.empty(originalValue) && originalValue === files}>
                <If condition={Is.array(originalValue)}>
                    <For array={originalValue} render={value => (
                        <HiddenInput name={name + 'String[]'} value={originalValue as string} />
                    )} />
                </If>
                <If condition={!Is.array(originalValue)}>
                    <HiddenInput name={name + 'String'} value={originalValue as string} />
