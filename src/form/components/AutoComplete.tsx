...
export default function AutoComplete(props: any) {
    ...
    let { label, limit, variant = config.get('form.input.variant', 'outlined'), disabled, except, request, loading: baseLoading, InputProps = {}, imageable, none, margin = 'normal', searchable, lazyLoading, groupBy, mapItem = mapItemInternally, addable = false, closeOnSelect, onChange, loadingText = trans('loading'), noOptionsText = trans('noOptions'), autoHighlight, items, name, value, multiple, required, mapResponse = defaultMapResponse, ...otherProps } = props;
