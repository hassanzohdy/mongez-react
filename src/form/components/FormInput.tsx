...
function Input(props: InputProps, ref) {
  const {
    onError = null,
    shrink = config.get("form.input.shrink", true),
    strict = config.get("form.input.strict", true),
    defaultValue = "",
    theme = config.get("form.input.theme"),
    icon,
    id = Random.id(),
    iconPosition = "start",
  ...
  const [isRequired, setRequiredState] = React.useState<boolean>(
    Boolean(required)
  );
  ...
  const validateInput = (e) => {
    ...
    if (inputValidation) {
      formInput.markAsInvalid(
        inputValidation.errorType,
  ...
  let label = trans(
    config.get("form.input.interchangeableLabelPlaceholder") === true
  ...
  if (theme === "bootstrap") {
    ...
    return (
      <FormControl
        ...
        className={clsx(
          classes.margin,
          Obj.get(otherProps, "classes.formControl")
        )}
      >
        <InputLabel shrink={shrink} htmlFor={otherProps["id"]}>
          {label}
        </InputLabel>
        ...
        {icon !== undefined && (
          <Box display="flex" mr={4} mt={3}>
            ...
            <Box mt={1.5} ml={-7.5} zIndex={1000} width={30}>
              {icon}
            </Box>
          </Box>
        )}
  ...
  if (theme === "redit") {
    return (
      <RedditTextField
        error={Boolean(error)}
        label={<Label label={label} shrink={shrink} required={isRequired} />}
