...
import { InputTheme, InputRule } from './../../../utils/types';
import TextField, { TextFieldProps } from '@material-ui/core/TextField';
import { Label, Input as SemanticInput, Form as SemanticForm } from 'semantic-ui-react'
import { Box, createStyles, alpha, FormControl, InputAdornment, InputBase, InputLabel, makeStyles, OutlinedInputProps, Theme, withStyles } from '@material-ui/core';
...
function Input(props: InputProps, ref) {
    const { labelPosition = 'top', onError = null, strict = config.get('form.input.strict', true), defaultValue = '', theme = getFormInputTheme(), icon, id = Random.id(), iconPosition = 'start', InputProps: baseInputProps, rules = config.get('form.input.rulesList', rulesList), value, readOnly, required, variant = config.get('form.input.variant', 'outlined'), onInput, name, ...otherProps } = props;
    const [isControlledComponent] = React.useState(!Is.undefined(value) && value !== null);
    ...
    if (theme === 'bootstrap') {
        ...
        return (
            <FormControl fullWidth className={clsx(classes.margin, Obj.get(otherProps, 'classes.formControl'))}>
                ...
                <InputError classes={props.classes} error={error} />
            </FormControl>
        )
    }
